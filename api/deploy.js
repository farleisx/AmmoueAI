  ...
  return sanitizeHtml(html, {
    allowedTags:
      plan === "free"
        ...
        : ["html","head","body","div","span","p","h1","h2","h3","ul","li","img","a","strong","em","b","i","table","tr","td","th"], // tighter than defaults
    allowedAttributes:
      plan === "free"
...
// Validate domains strictly (block IDNs, vercel domains, subdomains)
function validateDomain(domain) {
  if (!/^[a-z0-9-]+\.[a-z]{2,}$/i.test(domain)) return false; // only ascii, no IDN
...
export default async function handler(req, res) {
  try {
    ...
    // ----- Cooldown (transaction for concurrency) -----
    ...
    // ----- Deployment limit (only live deployments) -----
    ...
    if (slug) {
      ...
      try {
        await db.runTransaction(async (tx) => {
          ...
          if (snap.exists && snap.data().expiresAt.toMillis() > Date.now()) throw new Error("SLUG_TAKEN");
    ...
    // ----- Vercel deployment with retries -----
    let deployment;
      ...
      try {
        const deployRes = await fetch(`https://api.vercel.com/v13/deployments${VERCEL_TEAM_ID ? `?teamId=${VERCEL_TEAM_ID}`:""}`, {
          ...
          body:JSON.stringify({
            ...
            ...(finalSlug && {alias:[`${finalSlug}.vercel.app`]})
          })
        });
    ...
    await projectRef.update({ deploymentId: deployment.id, deploymentUrl: `https://${deployment.url}`, slug: finalSlug || null, status: "live", updatedAt: FieldValue.serverTimestamp() });

    if (slugRef) await slugRef.delete();

    return res.status(200).json({ deploymentId: deployment.id, deploymentUrl: `https://${deployment.url}`, slug: finalSlug });

  } catch (err) {
    console.error("Deploy error:", err.status || err.message || err);
