...
export default async function handler(req, res) {
  ...
  /* ---------------- FETCH ALL PAGES FOR DEPLOYMENT (UPDATED) ---------------- */
  let vercelFiles = [];
  try {
    ...
    if (projectSnap.exists) {
      ...
      Object.entries(pages).forEach(([name, data]) => {
        // Extract content whether it's a string or the {content: ""} object from the generator
        const fileData = typeof data === 'string' ? data : (data.content || "");
        
        // Use the original filename generated by AI (like package.json)
        let fileName = name;

        // Multi-page logic for UI files
        if (fileName === "landing") {
        ...
        // Also provide landing.html so internal <a> links don't break in vanilla
