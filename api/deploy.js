  ...
  try {
    ...
    if (projectSnap.exists) {
      ...
      Object.entries(pages).forEach(([name, content]) => {
        ...
        vercelFiles.push({ file: fileName, data: typeof content === 'string' ? content : (content.html || "") });
        
        // Also provide landing.html so internal <a> links don't break
        if (name === "landing") {
          vercelFiles.push({ file: "landing.html", data: typeof content === 'string' ? content : (content.html || "") });
        }
      });
    } else {
      // Fallback to single file if no project doc exists yet
      vercelFiles.push({ file: "index.html", data: htmlContent });
    }
  } catch (err) {
