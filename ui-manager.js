...
export function renderProjectHistory(listEl, projects, loadCallback) {
    ...
    projects.forEach(p => {
        ...
        item.className = "history-item p-4 rounded-2xl bg-white/5 hover:bg-[#D4AF37]/10 transition cursor-pointer relative group";
        item.innerHTML = `
            <div class="text-xs font-semibold text-white truncate pr-6">${p.title || p.prompt || "Untitled Project"}</div>
...
export function exportProject(projectState) {
    ...
    Object.entries(projectState.pages).forEach(([name, html]) => {
        folder.file(name.endsWith('.html') ? name : `${name}.html`, html);
    });
...
export function showNotification(message, type = "success") {
    ...
    toast.classList.add(type === "success" ? "bg-emerald-500/10" : "bg-red-500/10", type === "success" ? "border-emerald-500/50" : "border-red-500/50", type === "success" ? "text-emerald-400" : "text-red-400");
    toast.innerHTML = `<i class="fa-solid ${type === 'success' ? 'fa-circle-check' : 'fa-triangle-exclamation'}"></i> <span class="text-xs font-black uppercase tracking-widest">${message}</span>`;
...
export function showExportModal(projectState, onConfirm) {
    ...
    const filesHtml = Object.keys(projectState.pages).map(name => `<div class="flex items-center gap-3 text-gray-400 text-xs py-1 border-b border-white/5"><i class="fa-solid fa-file-code text-[#D4AF37]"></i> ${name.endsWith('.html') ? name : name + '.html'}</div>`).join('');
    modal.innerHTML = `
        <div class="bg-[#111113] border border-white/10 rounded-3xl max-w-md w-full p-8 shadow-2xl scale-95 opacity-0 transition-all duration-300" id="export-inner">
            <h3 class="text-xl font-black text-white mb-2 tracking-tight uppercase italic">Prepare Bundle</h3>
            <div class="bg-black/40 rounded-2xl p-4 border border-white/5 mb-8">
                <div class="flex items-center gap-2 text-white text-xs font-bold mb-3"><i class="fa-solid fa-folder-open text-[#D4AF37]"></i> Project_Assets/</div>
                <div class="pl-6">${filesHtml}</div>
            </div>
            <div class="flex gap-4">
                <button id="close-export" class="flex-1 text-gray-500 text-[10px] font-black uppercase tracking-widest">Cancel</button>
                <button id="confirm-export" class="flex-1 btn-luxury py-4 rounded-2xl text-[10px]">DOWNLOAD</button>
            </div>
        </div>
    `;
    ...
    modal.querySelector('#close-export').onclick = () => { modal.remove(); };
    modal.querySelector('#confirm-export').onclick = () => { onConfirm(); modal.remove(); };
}

export function initProjectSearch(searchEl, listEl, projects, loadCallback) {
    searchEl.oninput = (e) => {
        const term = e.target.value.toLowerCase();
        const filtered = projects.filter(p => (p.title || "").toLowerCase().includes(term) || (p.prompt || "").toLowerCase().includes(term));
...
export function updateCollabBadge(count) {
    ...
    if (!badge) {
        ...
        badge.className = 'fixed top-20 right-6 bg-emerald-500/10 border border-emerald-500/30 px-3 py-1.5 rounded-full flex items-center gap-2 z-[200] animate-bounce';
        document.body.appendChild(badge);
    }
    badge.innerHTML = `<div class="w-2 h-2 bg-emerald-500 rounded-full animate-pulse"></div><span class="text-[9px] font-black text-emerald-400 uppercase tracking-tighter">${count} LIVE VIEWERS</span>`;
